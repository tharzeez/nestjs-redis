...
export interface RedisClient {
  defaultKey: string;
  clients: Map<string, Graph>;
  // clients: Map<string, Redis.Redis>;
  size: number;
}

async function getClient(options: RedisModuleOptions): Promise<Graph> {
  const { onClientReady, url, host, graphId, port, ...opt } = options;
  // const client = url ? new Redis(url) : new Redis(opt);
  const client = new Graph(graphId, host, port, opt);
  if (onClientReady) {
...
export const createClient = (): Provider => ({
  ...
  useFactory: async (options: RedisModuleOptions | RedisModuleOptions[]): Promise<RedisClient> => {
    const clients = new Map<string, Graph>();
